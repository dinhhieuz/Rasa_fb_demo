...
ALTER PROCEDURE proc_chatbot_create_luong_cang (
	@id_fb varchar(100),
	@status varchar(100)
) AS
BEGIN
...
/*		
		...
		delete from zalo_salary_send
		select * from users
*/
--		select * from zalo_luong

insert into dsa_hieu_salary
            values ('1','4857932260991004',5,N'Trần Đình Hiếu','01.01.00.00',N'Chủ tịch',617867275,115343347,216550900,2022,4)
update dbo.users
set act_permission = 'act_dthu, act_dthu_time, act_dthu_time_req, act_dthu_store, act_dthu_store_req'
where stt = 4
----
select * from users
where stt = 4
...
select * from zalo_user where id_zalo = '8668477534363029464'
select * from users
select * from zalo_link_fb
SELECT id_zalo FROM zalo_link_fb WHERE id_fb = '4857932260991004'
select t_hsPCBS_KXĐ from zalo_luong	
...
BEGIN
	...
	BEGIN
		...
		declare @total bigint = round(rand()*(100-70+70)+70,3)*1000000
		declare @decline bigint = round(rand()*(50-10+10)+10,3)*1000000
		declare @residual bigint = @total - @decline
		declare @insurance bigint = round(rand()*(10-1+1)+1,3)*1000000
		DECLARE @input_data nvarchar(max) = N'
			...
			, LEFT(FORMAT( ('+convert(varchar(20),@total)+') , ''N'', ''vi-VN''), (LEN(FORMAT( '+convert(varchar(20),@total)+', ''N'', ''vi-VN''))-3)) AS [total] 
			...
			,[t_pctn], convert(float, 4.5) as [t_hsPCBS_KXÐ]  ,[t_htt],[t_hsdv],[t_np],[t_ncd],[t_cd]
			, LEFT(FORMAT( ('+convert(varchar(20),@decline)+') , ''N'', ''vi-VN''), (LEN(FORMAT( '+convert(varchar(20),@decline)+', ''N'', ''vi-VN''))-3)) AS [decline]
			...
			, LEFT(FORMAT( ('+convert(varchar(20),@insurance)+') , ''N'', ''vi-VN''), (LEN(FORMAT( '+convert(varchar(20),@insurance)+', ''N'', ''vi-VN''))-3)) AS [insurance]
			...
			, LEFT(FORMAT( ('+convert(varchar(20),@residual)+') , ''N'', ''vi-VN''), (LEN(FORMAT( '+convert(varchar(20),@residual)+', ''N'', ''vi-VN''))-3)) AS [residual]
			,l.status
			,s.status as status_send
		FROM zalo_luong l 
				left join zalo_user u ON l.id_nv = u.id_user
				left join zalo_salary_send s ON l.id_l = s.id_salary 
		WHERE 
			l.status = ''done''
...
for i in range(len(df)):
    month = df["date_l"][i].month
    year = df["date_l"][i].year
    print(df["total"][i])
    print(df["decline"][i])
    print(df["insurance"][i])
    print(df["residual"][i])
    print(df["total"][i])
    html = f"""
    <div>
    ...
    <center class="header" >
        ...
        <i><p style="font-size: 222%; line-height: 1.5;">Kính gửi: {df["gender"][i]} {df["name_u"][i]}</p></i>
    ...
    <table class = "content" style="width:85%"  cellpadding="9">
        ...
        <thead >
            <tr>
                <td>Mức lương đóng BHXH</td> <td>{df["t_mldBHXH"][i]}</td>
                <td>PC nặng nhọc, độc hại</td> <td>{df["t_pcnn"][i]}</td>
            </tr>
            <tr>
                <td>Phụ cấp trách nhiệm</td> <td>{df["t_pctn"][i]}</td>
                <td>Lương và PC đóng BHXH</td> <td>{df["t_lpcdBHXH"][i]}</td>
            </tr>
            <tr>
                <td>Hệ số PCBS KXĐ</td> <td>4.3</td>
                <td>PCBS KXĐ kiêm nhiệm</td> <td>{df["t_pcbskxdkn"][i]}</td>
            </tr>
            <tr>
                <td>Hạng thành tích</td> <td>{df["t_htt"][i]}</td>
                <td>KPI đơn vị</td> <td>{df["t_kpidv"][i]}</td>
            </tr>
            <tr>
                <td>Hệ số đơn vị</td> <td>{df["t_hsdv"][i]}</td>
                <td>Công chính</td> <td>{df["t_cc"][i]}</td>
            </tr>
            <tr>
                <td>Nghỉ phép</td> <td>{df["t_np"][i]}</td>
                <td>Nghỉ lễ, tết</td> <td>{df["t_nl"][i]}</td>
            </tr>
            <tr>
                <td>Nghỉ chế độ</td> <td>{df["t_ncd"][i]}</td>
                <td>Ngoài giờ</td> <td>{df["t_ng"][i]}</td>
            </tr>
            <tr>
                <td>Ca đêm</td> <td>{df["t_cd"][i]}</td>
                <td>Cách ly</td> <td>{df["t_cl"][i]}</td>
            </tr>
            <!-------TỔNG LƯƠNG VÀ KHOẢN TRỪ----->
            <tr>
                <th style="background-color: #83ccab;">Tổng thu nhập</th>
                <th style="background-color: #83ccab;">{df["total"][i]}</th>
                <th style="background-color: #f2b69b;">Các khoản giảm trừ</th>
                <th style="background-color: #f2b69b;">{df["decline"][i]}</th>
            </tr>
            <!------------>
            <tr>
                <td><b>Lương</b></td> <td></td>
                <td>Tạm ứng kỳ 1</td> <td>{df["d_tuk1"][i]} </td>
            </tr>
            <tr>
                <td>Lương chính</td> <td>{df["t_lc"][i]}</td>
                <td>Công đoàn phí</td> <td>{df["d_cdp"][i]}</td>
            </tr>
            <tr>
                <td>Lương nghỉ phép</td> <td>{df["t_lnp"][i]}</td>
                <td>Bảo hiểm xã hội (8%)</td> <td>{df["d_bhxh"][i]}</td>
            </tr>
            <tr>
                <td>Lương nghỉ lễ, tết</td> <td>{df["t_lnl"][i]}</td>
                <td>Bảo hiểm y tế (1,5%)</td> <td>{df["d_bhyt"][i]}</td>
            </tr>
            <tr>
                <td>Lương nghỉ chế độ</td> <td>{df["t_lncd"][i]}</td>
                <td>Bảo hiểm thất nghiệp (1%)</td> <td>{df["d_bhtn"][i]}</td>
            </tr>
            <tr>
                <td>Lương ngoài giờ</td> <td>{df["t_lng"][i]}</td>
                <td>Thuế thu nhập cá nhân</td> <td>{df["d_ttncn"][i]}</td>
            </tr>
            <tr>
                <td>Lương ca đêm</td> <td>{df["t_lcd"][i]}</td>
                <td>Thu tiền BHSK người thân</td> <td>{df["d_ttbhsknt"][i]}</td>
            </tr>
            <tr>
                <td>Lương cách ly</td> <td>{df["t_lcl"][i]}</td>
                <td>Bồi thường</td> <td>{df["d_bt"][i]}</td>
            </tr>
            <tr>
                <td>Phục vụ tàu Ấn Độ</td> <td>{df["t_pvtad"][i]}</td>
                <td>Truy thu</td> <td>{df["d_tt"][i]}</td>
            </tr>
            <tr>
                <td>TC LĐ nữ có con nhỏ</td> <td>{df["t_tcldnccn"][i]}</td>
                <td></td> <td></td>
            </tr>
            <tr>
                <td>Truy lĩnh</td> <td>{df["t_tl"][i]}</td>
                <td></td> <td></td>
            </tr>
            <tr>
                <td>Khác (Họp)</td> <td>{df["t_others"][i]}</td>
                <td></td> <td></td>
            </tr>
            <tr>
            ...
            <tr>
                <td>Hỗ trợ Điện thoại</td> <td>{df["t_htdt"][i]}</td>
                <td></td> <td></td>
            </tr>
            <tr>
                <td>Hỗ trợ Xăng xe</td> <td>{df["t_htxx"][i]}</td>
                <td></td> <td></td>
            </tr>
            <tr>
                <td>Hỗ trợ Đi lại</td> <td>{df["t_htdl"][i]}</td>
                <td></td> <td></td>
            </tr>
            <tr>
                <td>Hỗ trợ Nhà ở</td> <td>{df["t_htno"][i]}</td>
                <td></td> <td></td>
            </tr>
            <tr>
                <td>PCBS không xác định</td> <td>{df["t_pcbskxd"][i]}</td>
                <td></td> <td></td>
            </tr>
            <tr>
                <td>Tiền ăn giữa ca</td> <td>{df["t_tagc"][i]}</td>
                <td></td> <td></td>
            </tr>
            <tr>
                <td>Chống nóng</td> <td>{df["t_cn"][i]}</td>
                <td></td> <td></td>
            </tr>
            <tr>
                ...
                <td style="background-color: #edb528;"><b>{df["residual"][i]}</b></td>
            </tr>
    ...
    <div class="end" >
        <p style="line-height: 1.7; font-size: 120%; margin-left: 80px;"> 
            Chân thành cảm ơn sự đóng góp của <b>{df["gender"][i]} {df["name_u"][i]} </b>! 
    ...
    image = requests.post(url = HCTI_API_ENDPOINT, data = data, auth=(HCTI_API_USER_ID, HCTI_API_KEY)).json()["url"]
    
    #SEND IMAGE
    access_token = "YCTu0IdReaU5ndi59DcZGuNFSm8hpyWAq9D03mxjqKZrcrDn2u7RMQJ2UdvMcz1sbj4DOs6Ljr6Vx3jkKOhXKe3ZNsrOZ-uii-P9FnYhpdVrjM4pAEdQ4uRoG5jcxUT4ZOLfSdpcuJ6ffZfrVSQ-MelG4LbHbhbedk8X84Y9cHYZrX8k3S6TRkQ19oWBuAKquhPrAo-jrXxmjKuRKVYJ6xsq82vgrBu9myCJ5psXbZ7cu3uYE_g4QysQD5aS-uuGw8KT8qJXh6UadHfDJSUAPREyAMLhf89MWjyZQLUUXMx--0TSLvgBNlhMEny8hgjS_EChGpIhaLN4xWrfD9akwN0bnw19"
    headers = { "access_token": access_token }
    #---------------SEND IMAGE TO USER
    end_point = "https://openapi.zalo.me/v2.0/oa/message"
    user_id = df["id_zalo"][i]
    text = f"""BẢNG LƯƠNG CHI TIẾT THÁNG {month} NĂM {year} - {df["gender"][i]} {df["name_u"][i]} (Mã: {df["id_nv"][i]})"""
    payload = { 
        "recipient": {
        "user_id": user_id
        },
    ...
    if response.ok:
        ...
        status = "send failed" 
    time = datetime.now()
    df_check.append({"id_send":i, "id_zalo": df["id_zalo"][i], "id_salary": df["id_l"][i], "time_send": time, "status": status})
OutputDataSet = pd.DataFrame.from_dict(df_check, orient="columns")
		...
		DECLARE @id_send int = (select ISNULL(max(id_send),1) from zalo_salary_send)
